...
import { AuthService } from '../service/auth-sercice/auth.service'; // Asegúrate de importar tu AuthService
...
const permissionsGuard: CanActivateFn = (route, state) => {
  ...
  const router = new Router(); // Crea una instancia del Router o inyéctalo si es posible
  ...
  return authService.getUserRolesAndPermissions().pipe(
    map(data => {
      const hasAccess = data.permissions.includes(section);
      if (!hasAccess) {
